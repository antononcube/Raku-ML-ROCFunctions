...
#my @funcs = (roc-functions() (-) &AUROC).keys;
say @funcs.raku;
my @rocRes = @rocs.map( -> $r {
    @funcs.map({ $_.name => $_($r) }).Hash
});
say to-pretty-table(@rocRes, field-names => ["PPV", "NPV", "TPR", "ACC", "SPC", "MCC"]);

#my @rocRes2 = @funcs.map({ $_(@rocs) });
#say @rocRes2 ~~ Positional and @rocRes2.all ~~ Positional;
#say to-pretty-table(transpose( @funcs>>.name Z=> @rocRes2 ));

say '=' x 120;

say to-roc-hash( 'True', 'False', <True True False>, <False True False>);
